...
class DeleteArticleUseCaseTest{
    ...
    fun setup(){
        ...
        deleteArticleUseCase = DeleteArticleUseCase(fakeNewsApiRepository)
        insertArticleUseCase = SaveNewsUserCase(fakeNewsApiRepository)
    }


    @Test
    fun `testar deletar usecase`() = runTest {
        val article = Article(
            articleId = 1,
            author = "Bruno Nunes",
        ...
        Truth.assertThat(fakeNewsApiRepository.savedArticles).isEmpty()
